...
export function Dashboard() {
  ...
  const recentTasks = tasks.filter(task => task.status !== 'done').sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).slice(0, 5);
  const upcomingDeadlines = tasks.filter(task => task.due_date && task.status !== 'done').sort((a, b) => new Date(a.due_date!).getTime() - new Date(b.due_date!).getTime()).slice(0, 5);
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card className="glass-card">
              ...
              <CardContent className="space-y-3">
                {recentTasks.length === 0 ? <p className="text-muted-foreground text-center py-4">
                    Nenhuma tarefa pendente
                  </p> : recentTasks.map(task => <div key={task.id} className="flex items-center justify-between p-3 rounded-lg bg-white/5 border border-white/10">
            ...
            <Card className="glass-card">
              ...
              <CardContent className="space-y-3">
                {upcomingDeadlines.length === 0 ? <p className="text-muted-foreground text-center py-4">
                    Nenhum prazo próximo
                  </p> : upcomingDeadlines.map(task => {
                const daysLeft = getDaysUntilDeadline(task.due_date!);
                return <div key={task.id} className="flex items-center justify-between p-3 rounded-lg bg-white/5 border border-white/10">
                        <div className="flex-1 min-w-0">
                          <h4 className="font-medium truncate">{task.title}</h4>
                          <Badge variant="outline" className={priorityColors[task.priority]}>
                            {task.priority}
        ...
        <TabsContent value="tasks" className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredTasks.map(task => <Card key={task.id} className="glass-card">
                <CardContent className="p-4">
                  ...
                  {task.description && <p className="text-sm text-muted-foreground mb-3 line-clamp-2">
                      {task.description}
                    </p>}
        ...
        <TabsContent value="projects" className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredProjects.map(project => <Card key={project.id} className="glass-card">
                <CardContent className="p-4">
                  ...
                  {project.description && <p className="text-sm text-muted-foreground mt-2 line-clamp-2">
                      {project.description}
                    </p>}
                  {project.due_date && <p className="text-xs text-muted-foreground mt-2">
        ...
        <TabsContent value="goals" className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredGoals.map(goal => <Card key={goal.id} className="glass-card">
                <CardContent className="p-4">
                  ...
                  {goal.description && <p className="text-sm text-muted-foreground mb-3 line-clamp-2">
                      {goal.description}
                    </p>}
                  {goal.due_date && <p className="text-xs text-muted-foreground">
